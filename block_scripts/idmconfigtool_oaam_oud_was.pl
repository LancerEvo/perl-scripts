...
sub InputFileGen_prepareIDStore_OAAM
{
  my ($operation, $MSname) = (@_);
  $InputFileGen_prepareIDStore_OAAM = "${WORKDIR}${DIRSEP}InputFileGen_prepareIDStore_OAAM.conf";
  if ( ! open (FILE, ">$InputFileGen_prepareIDStore_OAAM") ) {
    print "ERROR: failed to write to $InputFileGen_prepareIDStore_OAAM\n";
    $exit_value = 1;
  } else {
  ...
  return $InputFileGen_prepareIDStore_OAAM;
}
...
sub ScriptGen_prepareIDStore_OAAM
{
  my ($operation, $MSname) = (@_);
  $ScriptGen_prepareIDStore_OAAM = "${WORKDIR}${DIRSEP}ScriptGen_prepareIDStore_OAAM.cfg";
  if ( ! open (FILE, ">$ScriptGen_prepareIDStore_OAAM") )
  {
    print "ERROR: failed to write to $ScriptGen_prepareIDStore_OAAM\n";
    $exit_value = 1;
  }
  else
  {
    print FILE "#!/usr/bin/expect -f\n";
    print FILE "spawn $IDM_HOME/idmtools/bin/idmConfigTool.sh -prepareIDStore mode=OAAM input_file=$InputFileGen_prepareIDStore_OAAM log_level=ALL log_file=${WORKDIR}${DIRSEP}prepareIDStore_OAAM.out dump_params=true\n";
    print FILE "set timeout -1\n";
    print FILE "expect {\n";
    print FILE "  -timeout -1\n";
    print FILE "  \"Enter ID Store Bind DN password :\" { send \"welcome1\\r\"; exp_continue }\n";
    print FILE "  \"Enter User Password for oaamAdminUser:\" { send \"welcome1\\r\"; exp_continue }\n";
    print FILE "  \"Confirm User Password for oaamAdminUser:\" { send \"welcome1\\r\"; exp_continue }\n";
    print FILE "  eof { puts \" \$expect_out(buffer) system prompt match! successfully exit prepareIDStore_OAAM!!\"; return }\n";
    print FILE "  }\n";
    close (FILE);
    `chmod 755 $ScriptGen_prepareIDStore_OAAM`
  }
  return $ScriptGen_prepareIDStore_OAAM;
}
...
sub ScriptGen_prepareIDStore_WAS
{
  ...
  {
    print FILE "#!/usr/bin/expect -f\n";
    print FILE "spawn $IDM_HOME/idmtools/bin/idmConfigTool.sh -prepareIDStore mode=WAS input_file=$InputFileGen_prepareIDStore_WAS log_level=ALL log_file=${WORKDIR}${DIRSEP}prepareIDStore_WAS.out dump_params=true\n";
    print FILE "set timeout -1\n";
    print FILE "expect {\n";
    print FILE "  -timeout -1\n";
    print FILE "  \"Enter ID Store Bind DN password :\" { send \"welcome1\\r\"; exp_continue }\n";
...
sub print_test_parameter
{
  ...
  print "&&&&&&&&&&&&&Below is the result for Result_prepareIDstore_OAM !&&&&&&&&&&&&&&&&&&&&&&&&&&&&    $Result_prepareIDstore_OAM \n";
    $Result_prepareIDstore_OAAM = `$ScriptGen_prepareIDStore_OAAM`;
  print "==========================================================";
  print "&&&&&&&&&&&&&Below is the result for Result_prepareIDstore_OAAM !&&&&&&&&&&&&&&&&&&&&&&&&&&&&    $Result_prepareIDstore_OAAM \n";
  sleep 30;
}
...
sub install
{
  ...
  InputFileGen_prepareIDStore_OAAM();
  
  IDMconfigScriptGen_preConfigIDStore();
  ScriptGen_prepareIDStore_OAAM();
